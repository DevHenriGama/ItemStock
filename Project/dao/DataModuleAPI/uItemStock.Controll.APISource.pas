...
type
  TApiData = class(TServerMethodDataModule)
    ...
    procedure EventsMainEventslistallReplyEvent(var Params: TDWParams;
      var Result: string);
    procedure EventsMainEventsgettypesReplyEvent(var Params: TDWParams;
      var Result: string);
...
procedure TApiData.EventsMainEventsgettypesReplyEvent(var Params: TDWParams;
  var Result: string);
  var DAOTypes : IDAOTypesInstruction;   JObj ,JManipulator : TJSONObject; JArray : TJSONArray;
    I: Integer;  TempList : TStringList;
begin
  DAOTypes := TDAOTypesInstructon.Create;
  
  JObj := TJSONObject.Create;
  JArray := TJSONArray.Create;
  
  TempList := DAOTypes.GetListAllTypes;
    with TempList do begin  
      for I := 0 to Count -1  do begin
          JManipulator := TJSONObject.Create;
          JManipulator.AddPair('type',TempList[I]);
          JArray.Add(JManipulator);
      end;
      
        JObj.AddPair('total_types',IntToStr(Count));
        JObj.AddPair('types',JArray);
    end;
