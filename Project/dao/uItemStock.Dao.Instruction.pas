...
type
  ...
  public
    ...
    class function NewInstruction(MyClassItem: TItem): IDaoInstruction; deprecated;
...
begin
 with FQueryView do begin
  Close;
  SQl.Clear;
  sql.Add('SELECT * FROM ITENS ORDER BY ITEMNAME');
  Open;
 end;
...
begin
 with FQueryView do begin
  Close;
  SQl.Clear;
  sql.Add('SELECT * FROM ITENS WHERE ITEMNAME LIKE ' + QuotedStr(SearchItem + '%'));
  Open;
 end;
